...
static CPValue meetValue(CPValue v1, CPValue v2) {
    ...
    if (v1.kind == NAC || v2.kind == NAC)
        ...
        return v2.kind == UNDEF ? get_UNDEF() : get_CONST(v2.const_val);
    else return get_CONST(v1.const_val);
}
...
static CPValue calculateValue(IR_OP_TYPE IR_op_type, CPValue v1, CPValue v2) {
    ...
    else if(v1.kind == CONST) return v2.kind==UNDEF?get_UNDEF():get_NAC();   
    else return v1.kind==UNDEF?get_UNDEF():(v2.kind==UNDEF?get_UNDEF():get_NAC());
}
...
void ConstantPropagation_transferStmt (ConstantPropagation *t,
    ...
    if(stmt->stmt_type == IR_ASSIGN_STMT) {
        ...
        CPValue use_val = Fact_get_value_from_IR_val(fact, assign_stmt->rs),old_val = Fact_get_value_from_IR_var(fact, def);
        ...
        if (old_val.kind != use_val.kind || old_val.const_val != use_val.const_val)
            use_val.kind == UNDEF?VCALL(*fact, delete, def):VCALL(*fact, set, def, use_val);
         ...
         */
        CPValue use_val = calculateValue(IR_op_type, rs1_val, rs2_val),old_val = Fact_get_value_from_IR_var(fact, def);
        if (old_val.kind != use_val.kind || old_val.const_val != use_val.const_val)
            use_val.kind == UNDEF?VCALL(*fact, delete, def):VCALL(*fact, set, def, use_val);
